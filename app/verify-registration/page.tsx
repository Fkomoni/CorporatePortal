...
function VerifyForm() {
  ...
  // Sync if URL params arrive after hydration
  useEffect(() => { if (urlCode && !code) setCode(urlCode); }, [urlCode]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      {success ? (
        ...
        <>
          <div style={{ marginBottom: 28 }}>
            <p style={{ fontSize: 22, fontWeight: 800, color: '#131C4E', letterSpacing: '-0.02em', marginBottom: 6 }}>
              {codeFromUrl ? 'Set Your Password' : 'Verify Registration'}
            </p>
            <p style={{ fontSize: 13, color: '#9CA3B8', lineHeight: 1.5 }}>
              {codeFromUrl
                ? 'Your identity is verified. Create a password to complete your account setup.'
                : 'Enter the verification code sent to your email and create your new password.'}
            </p>
          </div>

          {/* Email pill — shown if known from URL */}
          {urlEmail && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F0F4FF', border: '1px solid #C7D2FE', borderRadius: 12, padding: '11px 16px', marginBottom: 22 }}>
              ...
              <div>
                <p style={{ fontSize: 10, fontWeight: 700, color: '#6366F1', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 1 }}>Signing up as</p>
                <p style={{ fontSize: 13, fontWeight: 600, color: '#131C4E' }}>{urlEmail}</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {/* Verification code — hidden (but present) when pre-filled via URL */}
            {codeFromUrl ? (
            ...
            <button
              type="submit"
              disabled={loading || (!codeFromUrl && !code) || !password || !confirm}
              style={{ height: 50, borderRadius: 14, border: 'none', background: 'linear-gradient(135deg,#F56B22,#FF8C4B)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', opacity: (loading || (!codeFromUrl && !code) || !password || !confirm) ? 0.6 : 1, boxShadow: '0 4px 14px rgba(245,107,34,0.36)', marginTop: 4, transition: 'opacity 0.15s' }}>
              {loading ? 'Setting up your account…' : codeFromUrl ? 'Create Account' : 'Verify & Set Password'}
            </button>
          </form>
