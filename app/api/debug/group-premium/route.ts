...
export async function GET(req: Request) {
  ...
  // groupId: prefer ?groupId= param (for testing), fall back to session companyId
  const groupId = searchParams.get('groupId') ?? session.user.companyId ?? '';
  if (!groupId) {
    return NextResponse.json({ error: 'No groupId: pass ?groupId= or ensure your account has a companyId' }, { status: 400 });
  }

  const url = `${BASE}/api/CorporateProfile/GetGroupPremium?groupid=${groupId}`;
  ...
  try {
    ...
    // Flatten every field we can find in the response for easy inspection
