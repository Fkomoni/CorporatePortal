            ...
            <div className="flex gap-2">
              {(['Gold Plus', 'Silver', 'Bronze'] as Plan[]).map((plan) => (
                <button key={plan} onClick={() => setActivePlan(plan)}
                  className={`px-4 py-2 rounded-xl text-[12px] font-semibold border transition-all ${activePlan === plan ? 'border-[#F56B22] text-[#F56B22] bg-[#FFF1E6]' : 'border-[#E5E7F1] text-[#6B7280] bg-white hover:border-[#9CA3B8]'}`}>
                  {plan}
                </button>
              ))}
            </div>
              ...
              {benefits[activePlan].map((b) => (
                <div key={b.category} className="bg-white rounded-2xl p-5 shadow-sm border border-[#F0F1F5]">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {b.waitingPeriod && (
                      <span className="text-[10px] font-semibold bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full">{b.waitingPeriod} wait</span>
                    )}
                  </div>
                  ...
                  <div className="space-y-1 mb-3">
                    ...
                    {b.includes.map((inc) => (
                      <div key={inc} className="flex items-center gap-1.5">
                        ...
                        <span className="text-[12px] text-[#3A4382]">{inc}</span>
                      </div>
                    ))}
                  </div>
                  {b.excludes.length > 0 && (
                    <div className="space-y-1 pt-3 border-t border-[#F7F8FA]">
                      ...
                      {b.excludes.map((exc) => (
                        <div key={exc} className="flex items-center gap-1.5">
                          <XCircle className="w-3 h-3 text-red-400 flex-shrink-0" />
                          <span className="text-[12px] text-[#9CA3B8]">{exc}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div key={p.name} className="bg-white rounded-2xl p-5 shadow-sm border border-[#F0F1F5]">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0"
                      style={p.status === 'Active' ? { background: '#ECFDF5', color: '#059669' } : { background: '#FEF2F2', color: '#DC2626' }}>
                      {p.status}
                    </span>
                  </div>
